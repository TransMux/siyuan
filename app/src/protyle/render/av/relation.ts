...
draggable="true">${genSelectItemHTML({
        ...
        cellElement.querySelectorAll(".av__celltext--ref").forEach((relationItem: HTMLElement) => {
            ...
            selectHTML += `<button data-id="${relationItem.dataset.id}" data-type="setRelationCell" class="b3-menu__item${relationItem.textContent.indexOf(keyword) > -1 ? "" : " fn__none"}" draggable="true">${genSelectItemHTML({
                type: "selected",
                id: relationItem.dataset.id,
                isDetached: !relationItem.classList.contains("av__celltext--ref"),
                text: Lute.EscapeHTMLStr(relationItem.textContent || window.siyuan.languages.untitled)
            })}</button>`;
        });
        cells.forEach((item) => {
...
draggable="true">${genSelectItemHTML({
        ...
        options.cellElements[0].querySelectorAll(".av__celltext--ref").forEach((relationItem: HTMLElement) => {
            ...
            hasIds.push(relationItem.dataset.id);
            selectHTML += `<button data-id="${relationItem.dataset.id}" data-type="setRelationCell" class="b3-menu__item" draggable="true">${genSelectItemHTML({
                type: "selected",
                id: relationItem.dataset.id,
                isDetached: !relationItem.classList.contains("av__celltext--ref"),
                text: Lute.EscapeHTMLStr(relationItem.textContent || window.siyuan.languages.untitled)
